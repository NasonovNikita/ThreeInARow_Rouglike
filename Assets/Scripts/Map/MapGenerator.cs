...
namespace Map
{
    public class MapGenerator : MonoBehaviour
    {
        ...
        private readonly Dictionary<int, List<EnemyGroup>> _groups = new();
    
        private readonly Dictionary<VertexType, int> _vertexesByChance = new ()
        {
            [VertexType.Battle] = 13,
            [VertexType.Shop] = 4
        };
        private int _vertexesFrequencySum;

        private Good[] _goods;
        private int _goodsFrequencySum;
        ...
        public KeyValuePair<List<List<VertexData>>, List<List<KeyValuePair<int, int>>>> GetMap(int seed)
        {
            ...
            {
                if (!_groups.TryAdd(group.Difficulty(), new List<EnemyGroup> {group}))
                {
                    _groups[group.Difficulty()].Add(group);
                }
            }

            _goods = Resources.LoadAll<Good>("Goods");
            foreach (Good good in _goods)
            {
                _goodsFrequencySum += good.frequency;
            }

            foreach (var vertex in _vertexesByChance)
            {
                _vertexesFrequencySum += vertex.Value;
            }
        ...
        private void PlaceLayer(IReadOnlyList<VertexData> layer, int i, Vector3 xStep, Vector3 yStep)
        {
            ...
            for (int j = 1; j < k; j++)
            {
                layer[j].position = layer[j - 1].position + xStep + Random.Range(-25, 26) * _dX + Random.Range(-25, 26) * _dY;
            }
        }
        ...
        private BattleVertexData GenBattle(int layer)
        {
            ...
            int chosenKey = _groups.Keys.Aggregate(
                (min, next) => Math.Abs(min - battleDifficulty) < Math.Abs(next - battleDifficulty) ? min : next
            );

        
            EnemyGroup group = Instantiate(_groups[chosenKey][Random.Range(0, _groups[chosenKey].Count)]);
            vertex.group = group;
        ...
        {
            int choice = Random.Range(0, _goodsFrequencySum);
            foreach (Good good in _goods)
            {
        ...
        {
            int choice = Random.Range(0, _vertexesFrequencySum);
